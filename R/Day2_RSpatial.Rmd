...
date: "`r Sys.Date()`" # "`r Sys.setlocale('LC_TIME','C'); paste('Last Update', format(Sys.time(), '%B %e, %Y')) `"
output:
  html_document:
    code_folding: show
    link-citations: yes
...
#knitr::opts_knit$set(root.dir = 'c:/Users/kramer/Dropbox/_Geodata_Borneo')
```
...
under development - called *terra* package - but the course does not yet cover this.
...
```{r}
pkgs = c("here", "terra", "sp", "sf", "dismo", "raster", "GISTools",  "rgdal", 
         "maptools", "rgeos","rgl","rasterVis", "ellipse", "units", "tmap", 
         "viridis","dplyr", "gdistance","vegan", 
...
Please note: if you follow the d6-package installation 
https://github.com/EcoDynIZW/d6
and have stored all your geo-data in the respective folder (.../data-raw/geo-raw),
there is no need to set the root folder. The root folder automatically
...
mypath <- here()
root_wd <- dirname(mypath) # dirname gives the parent/ root dir

# or set it 'by hand' (less elegant), e.g.
# root_wd <- setwd("C:/Users/kramer/PopDynIZW Dropbox/Steph Kramer/_GitHub")
```
<br>

Assign the subfolders relative to wd. (please adjust to your setup)

```{r}
#today, we start with Borneo
work_wd   <- paste0(root_wd, "/", "_DataBorneo") # mind difference paste0() and paste()
maps_wd   <-  paste(work_wd, "/", "BaseMaps",    sep = '')
recs_wd   <-  paste(work_wd, "/", "BaseRecords", sep = '')
```
<br>
Create an output folder where you can temporarily store your results with *dir.create*.
Mind: the folder structure already exists when using the d6-package. 
...
```
<br>
Projections can also be transferred from one object to another, when the crs hasn't yet been specified/ set . In the following, load the topographic map (digital elevation model) of Borneo (Bor_dem.asc) and assign it the same crs as Bor_mat:
```{r}
...
Create a hillshade (3D look) based on topography using slope and aspect:
...
small clipped/ cropped raster and plot it on top of the hillshade
...
# interactive plot
tmap_mode(mode = "view")
tm_shape(shp = hillsh, raster.downsample= TRUE)  + tm_raster(palette = "Greys") +
  tm_shape(shp = ras24, raster.downsample= TRUE) + tm_raster(palette = grDevices::topo.colors(20),
                                    alpha = 0.3) +
  tm_shape(shp = ras1, raster.downsample= TRUE) + tm_raster(palette = grDevices::rainbow(10),
                                    alpha = 0.3) +
  tm_shape(shp = ras1_cr_sf) + tm_dots(shape = 20, 
                                       size = 0.01) 
            ## shape arg: 20 is equal to o, in view mode  only dots and circles allowed
```
...
tmap_mode(mode = "plot")
tm_shape(shp = hillsh, raster.downsample= TRUE) + tm_raster(palette = "Greys") +
  tm_shape(shp = ras24, raster.downsample= TRUE) + tm_raster(palette = grDevices::terrain.colors(10),
                                     alpha = 0.3) +
  tm_shape(shp = ras1_cr_sf) + tm_dots(shape = 3, # 3 is equal to +
                                        size = 0.5) 
...
or simply in ggplot2. For this, we need to transform the data first:
```{r}
...
#take a subsample of the data to not crash your PC:
...
Changing cell size use aggregate() or resample().
```{r}
# collapse 10*10 cells into 1 using mean:
ras1_agg <- aggregate(x=ras1,fact=10, fun= mean) 
plot(ras1_agg)
```
...
# Change values e.g. for adding forest
